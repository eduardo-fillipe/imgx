...
class SpatialMask:
    ...
    def apply_mask(self, image_matrix: np.ndarray, max_pixel_color: int, img_color_type: ImageColorType) -> np.ndarray:
        """
        ...
        :param img_color_type: Color type of this image.
        ...
        if str(self.overlap_strategy.name).startswith("PADDING"):  # if the strategy is to Pad
            ...
            for i in range(m):
                for j in range(n):
                    image_matrix[i, j] = self._apply_on_pixel(padded_matrix, padding, img_color_type,
                                                              (i + padding, j + padding))
            return image_matrix
            ...
            for i in range(m):
                for j in range(n):
                    image_matrix[i, j] = self._apply_on_pixel_partially(w_image, padding, img_color_type, (i, j))
            ...
            for i in range(i_start, i_stop):
                for j in range(j_start, j_stop):
                    image_matrix[i, j] = self._apply_on_pixel(w_image, padding, img_color_type, (i, j))

            return image_matrix[i_start:i_stop, j_start:j_stop]

        return image_matrix

    def _apply_on_pixel(self, image_matrix: np.ndarray, padding: int, img_type: ImageColorType,
    ...
    def _apply_on_pixel_partially(self, image_matrix: np.ndarray, padding: int, img_type: ImageColorType,
                                  target_pixel: tuple[int, int]) -> object:
        """
...
class AverageSpatialMask(SpatialMask):
    def _apply_on_pixel(self, image_matrix: np.ndarray, padding: int, img_type: ImageColorType,
        ...
        if img_type == ImageColorType.RGB:
